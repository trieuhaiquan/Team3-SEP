...
class Model_mindex {
    ...
    public function workdone($iduser) {
        $data = array();
        $sql = "select time.id,idtag,timestart,timeend,status,priority,tentag,timestart2,timeend2,timeendreal,timestartreal,totaltime,totalestimate from managetime as time , managetags as tag where status =3 and time.idtag = tag.id and time.iduser = $iduser and anhien=0 order by priority ASC";
        if (!$kq = $this->db->query($sql))
            die($this->db->error);
        foreach ($kq as $row)
            $data[] = $row;
    ...
    public function xoadone($idtime, $iduser) {
        $sql = "UPDATE managetime SET anhien=1 WHERE id=$idtime and `iduser`='$iduser'";
        if (!$rs = $this->db->query($sql))
            die($this->db->error);
    }

    public function changepass($pass1, $iduser) {
        $sql = "UPDATE user SET `pass`='$pass1' WHERE `id`='$iduser'";
        if (!$rs = $this->db->query($sql))
            die($this->db->error);
    }
